...
function Notes() {
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            {/* 3.2. Form Nhập liệu */}
            <div style={{
                border: '1px solid #ccc', padding: '10px', marginBottom: '20px'
            }}>
                <h3>{formData.id ? 'Sửa ghi chú' : 'Thêm ghi chú mới'}</h3>
                <input
                    placeholder="Tiêu đề" value={formData.title}
                    onChange={e => setFormData({ ...formData, title: e.target.value })}
                    style={{ display: 'block', width: '100%', marginBottom: '10px' }}
                />
                <textarea
                    placeholder="Nội dung" value={formData.content}
                ...
                {formData.id && (
                    <button onClick={() => setFormData({ id: null, title: '', content: '' })}
                        style={{ marginLeft: '10px' }}>Hủy</button>
                )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                ...
                {notes.map(note => (
                    <div key={note.id} style={{
                        border: '1px solid #007bff', padding: '15px',
                        borderRadius: '5px'
                    }}>
